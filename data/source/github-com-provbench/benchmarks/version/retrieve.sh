...
version="$1"
version_reason=""
url="https://api.github.com/users/provbench/repos"
...
if [[ ! -d $version || ! -d $version/source || `find $version -empty -type d -name source` ]]; then
   ...
   pushd $version/source &> /dev/null
      ...
      if [[ "$url" =~ http* ]]; then                                                    # |
	curl -s -L -H "Accept: application/vnd.github.v3+json" https://api.github.com/users/provbench/repos > repos.json
	justify.sh https://api.github.com/users/provbench/repos repos.json http://dbpedia.org/resource/Hypertext_Transfer_Protocol
      fi                                                                                # | 
